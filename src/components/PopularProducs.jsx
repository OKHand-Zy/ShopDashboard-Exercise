...
function PopularProducs() {
  return (
    <div className='bg-white px-4 pt-3 pb-4 rounded-sm border border-gray-200 w-[20rem]'>
        <strong className='text-gray-700 font-medium'>Popular Producs</strong>
            ...
            {popularProductsData.map( product => (
                <Link to={`/product/${product.id}`} className='flex hover:no-underline'>
                    ...
                    <div className='ml-4 flex-1'>
                        ...
                        <span className={`text-sm font-medium ${product.product_stock === 0 ? 'text-red-600' : 'text-green-500'}`}>
